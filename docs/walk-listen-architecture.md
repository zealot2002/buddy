...
| 毒舌老炮 × 苏轼双人设能拉高停留与分享 | 围栏级 `primaryCompanionId` + 可手动切换 |
...
- 围栏触发时：`applyCompanion(payload.companionId)`，跟围栏 `primaryCompanionId` 对齐。
- 用户可在右上角手动切换旅伴；续读 TTS 使用卡片上的 `companionId`。
